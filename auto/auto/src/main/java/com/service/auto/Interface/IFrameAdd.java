...
public class IFrameAdd extends JFrame {
	...
	private static JComboBox cbCant;
	...
	public static JInternalFrame getAddForm() {
		...
		if (addForm == null || addForm.isClosed()) {
			...
			cbCant = new JComboBox(items);
			cbCant.setPreferredSize(new Dimension(200, 20));
			panelPO.add(cbCant);
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if (n == 0) {
						addOper.add(operatie.get(cbDenO.getSelectedIndex()));
						 sumPret += (operatie.get(cbDenO.getSelectedIndex()).getPret_oper());
						 tfPret.setText(Integer.valueOf(sumPret).toString());
					}
				}
			});
			...
			cbDenP.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					Object[] var = { "OK" };
					int n = JOptionPane.showOptionDialog(null,
							"Pentru a adauga piesa introduceti cite piese s-au folosit", "Confirmare",
							JOptionPane.YES_NO_OPTION,
							JOptionPane.QUESTION_MESSAGE, null, var, var[0]);
			...
			cbCant.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (n == 0) {
						...
						addPiesa.add(piese.get(cbDenP.getSelectedIndex()));
						 sumPret += ((piese.get(cbDenP.getSelectedIndex()).getPret()) * (cbCant.getSelectedIndex()));
						 tfPret.setText(Integer.valueOf(sumPret).toString());
					} 
				}
			});
