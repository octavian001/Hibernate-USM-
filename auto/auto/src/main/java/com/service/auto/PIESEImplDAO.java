...
public class PIESEImplDAO {
	...
	// extrag toate operatiile dupa ID_BON, pentru a putea incarca in tabela
		// principala toate piesele pentru un anumit ID_BON
		public ArrayList<PIESE> getAllPieseFK(int i) {
			ArrayList<PIESE> results = null;
			...
			try {
				org.hibernate.Query query = session.createQuery("SELECT "
						+ " p FROM PIESE p  JOIN p.bon b WHERE b.id_bon = :t ");
				query.setParameter("t", i);
				results = (ArrayList<PIESE>) query.list();
